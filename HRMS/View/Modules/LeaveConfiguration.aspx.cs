...
namespace HRMS.View.Modules
{
    public partial class LeaveConfiguration : System.Web.UI.Page
    {
        ...
        private List<ConfigRowItem> BuildInlineRows()
        {
            ...
            foreach (var type in types.OrderBy(x => x.TypeName))
            {
                var existingRow = existing.FirstOrDefault(x => x.TypeId == type.TypeId);
                merged.Add(existingRow ?? type);
            }
        ...
        protected void gvInlineConfig_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            ddlOption.Items.Add(new ListItem("-- Please Select --", string.Empty));
            foreach (var option in ApprovalOptions)
            {
                ddlOption.Items.Add(new ListItem(option.Text, option.Id.ToString()));
            }
            ...
            if (rowData.ApprovalOptionId > 0)
            {
                ...
                if (item != null)
                {
                    ddlOption.SelectedValue = rowData.ApprovalOptionId.ToString();
                }
            }
            ...
            using (MySqlConnection con = new MySqlConnection(ConnStr))
            {
                ...
                using (MySqlDataReader dr = cmd.ExecuteReader())
                {
                    if (dr.Read())
                    {
                        ...
                        lblMsg.Text = remarks;
                        lblMsg.CssClass = string.Equals(status, "Success", StringComparison.OrdinalIgnoreCase)
                            ? "text-success"
                            : "text-danger";
                    }
                }
