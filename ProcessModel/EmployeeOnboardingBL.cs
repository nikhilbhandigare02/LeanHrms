...
namespace ProcessModel
{
    public class EmployeeOnboardingBL
    {
        ...
        public EmployeeOnboardingResponseDO SaveEmployeeOnboarding(EmployeeOnboardingDO employee)
        {
            ...
            try
            {
                ...
                using (MySqlCommand cmd = new MySqlCommand("SP_SaveEmployeeOnboarding", con))
                {
                    ...
                    cmd.Parameters.AddWithValue("@p_PF_no", employee.PfNo);
        ...
        public EmployeeOnboardingResponseDO GetTextBoxValue(string type)  //sagar 10-07-2026
        {
            EmployeeOnboardingResponseDO response = new EmployeeOnboardingResponseDO
            {
                Status = "Failed",
            ...
            string sp_name = "";
            if (type == "txtEmployeeCode")
            {
                sp_name = "sp_GetNextEmployeeCode";
            }
            try
            {
                using (MySqlConnection con = new MySqlConnection(NormalizeMySqlConnectionString(Sqlconnection)))
                   
                using (MySqlCommand cmd = new MySqlCommand(sp_name, con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.CommandTimeout = 0;
                    ...
                    using (MySqlDataReader dr = cmd.ExecuteReader())
                    {
                        if (dr.Read())
                        {
                            //response.AutogeneratedEmpCode = ReadString(dr, "empcode");
                            response.AutogeneratedEmpCode = ReadString(dr, "@code");
                        }
                    }
                }
            }
            ...
            {
                CommonBL errorlog = new CommonBL();
                errorlog.fnStoreErrorLog("EmployeeOnboardingBL", "getAutoEMPCODE", "Exception Message: " + ex.Message + " StackTrace: " + ex.StackTrace, UserId);
                response.Message = "Employee onboarding get emp code failed due to an exception: " + ex.Message;
            }
